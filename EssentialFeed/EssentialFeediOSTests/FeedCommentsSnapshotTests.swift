...
class FeedCommentsSnapshotTests: XCTestCase {
	...
	func test_feedCommentsWithContent() {
		...
		assert(snapshot: sut.snapshot(for: .iPhone8(style: .dark, preferredContentSizeCategory: .extraExtraLarge)), named: "FEED_COMMENTS_WITH_CONTENT_extra_extra_large_dark")
	}
	...
	func test_feedCommentsWithErrorMessage() {
		...
		assert(snapshot: sut.snapshot(for: .iPhone8(style: .dark, preferredContentSizeCategory: .extraExtraLarge)), named: "FEED_COMMENTS_WITH_ERROR_MESSAGE_extra_extra_large_dark")
	}
